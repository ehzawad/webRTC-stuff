...
function send_message(){
    ...
    x = xhr.open('GET', 'http://192.168.10.82:5025/message?sender='+sender+'&output=' + encodeURIComponent(output), true);
...
//   }
    ...
    const keepAliveInterval = setInterval(() => {
        dc.send('keep-alive');  // Replace with your actual DataChannel send method
      ...
      pc.oniceconnectionstatechange = function () {
        ...
        if (pc.iceConnectionState === 'disconnected') {
            ...
            btn_show_start();
            isWebRTCConnected = false; // Reset the flag
        }
    };
    ...
    if (!isWebRTCConnected) {
        navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
            ...
            isWebRTCConnected = true; // Set the flag to true
            return negotiate();
        }, function (err) {
...
function stop() {
    ...
    if (globalStream) {
        let audioTracks = globalStream.getAudioTracks();
